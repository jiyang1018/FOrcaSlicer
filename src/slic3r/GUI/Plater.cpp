...
// FOS R1: CustomNotebook moved to Widgets/CustomNotebook.hpp so Tab.cpp can
// use it too (the raw-wxNotebook repaint storm). Included from the top of
// this file with the other headers.
    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(42 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
	...
	Choice::register_dynamic_list("wall_filament", &dynamic_filament_list_1_based);  // FOS: OW-identity - objects/features carry a concrete filament, so the feature dropdowns drop the "Default"(0) row and use the 1-based filament list. Support/tower stay gated.
...
void Plater::priv::set_project_name(const wxString& project_name)
{
    ...
    //update topbar title
#ifdef __WINDOWS__
    wxGetApp().mainframe->SetTitle(m_project_name + " - FOrcaSlicer");
