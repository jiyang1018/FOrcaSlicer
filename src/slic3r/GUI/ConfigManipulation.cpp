...
void ConfigManipulation::update_print_fff_config(DynamicPrintConfig* config, const bool is_global_config, const bool is_plate_config)
{
    ...
    double max_lh = gpreset.config.opt_float("max_layer_height", 0);
    {
        ...
        if (mx) {
            ...
            const bool sup = config->option<ConfigOptionBool>("enable_support") && config->opt_bool("enable_support");
...
void ConfigManipulation::toggle_print_fff_options(DynamicPrintConfig *config, const bool is_global_config)
{
    ...
    for (auto el : {"wall_filament", "inner_wall_filament", "sparse_infill_filament", "solid_infill_filament", "wipe_tower_filament"})
