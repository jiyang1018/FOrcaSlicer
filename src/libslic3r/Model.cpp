...
ModelObject* Model::add_object(const char *name, const char *path, const TriangleMesh &mesh)
{
    ...
    if (!new_object->config.has("extruder") || new_object->config.extruder() == 0)
        new_object->config.set_key_value("extruder", new ConfigOptionInt(0)); // FOS: keep default (0) so global Filament-for-Features is not overwritten by a forced object extruder
